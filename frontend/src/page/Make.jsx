...
const Make = () => {
  ...
  const createPlannerHandler = async () => {
    try {
      ...
      console.log(
        '자동 플래닝 생성을 데이터를 전송하였습니다: ',
        response.data,
      );
    } catch (error) {
      console.error(
        '자동 플래닝 생성을 데이터를 전송하지 못하였습니다: ',
        error,
      );
    }
  };
  ...
  return (
    <div className={styles.viewport}>
            ...
            {buttonStates.map((button, index) => (
              <button
                ...
                className={`${styles.datebtn} ${
                  button.pressed ? styles.btnpressed : ''
                }`}
      ...
      {addGoal && (
        <AddGoal onClose={closeGoalHandler} onAddGoal={addgoalHandler} />
      )}
      {addCourse && (
        <AddCourse
